...
def load_authenticator():
    ...
    # For production, load from secrets
            ...
            'credentials': {
                'usernames': {
                    'demo': {
                        ...
                        'name': 'Demo User',
                        'password': '$2b$12$iWXWpDHwkxP0LwVSiLVYVeJPT.EH3kV6kH3SSX8Y3SY2S2sT8QFQO'  # password: demo123
                    }
                }
            },
            'cookie': {
                'expiry_days': 30,
                'key': 'some_signature_key_that_should_be_changed',
                'name': 'task_assignment_auth'
            },
            'preauthorized': {
                'emails': []
            }
...
def show_landing_page():
    ...
    st.markdown('<p class="big-font">📋 Team Task Assignment Tool</p>', unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center;'>Efficiently manage task assignments across your team</h3>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        <p>Match tasks with team members based on languages and availability</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <p>Multiple team leads can work simultaneously without conflicts</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <p>Generate comprehensive analytics and track team utilization</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        st.info("🔐 **Team Leads**: Please login with your credentials to access real data")
    with col2:
        st.success("🎮 **Visitors**: Try Demo Mode to explore all features with sample data")
...
if st.session_state.get('demo_mode', False):
    ...
    with col1:
        st.warning('🎮 **Demo Mode** - Using sample data only. Changes will not be saved permanently.')
    with col2:
...
# If we reach here, user is authenticated (real or demo)
if st.session_state.authentication_status:
    
    # Configure data storage based on mode
    if st.session_state.get('demo_mode', False):
        ...
        # Show logout button for real users
        authenticator.logout('Logout', 'sidebar')
        st.sidebar.success(f'Welcome *{st.session_state.name}*')

    # GitHub API Functions (only for non-demo mode)
    ...
    def get_data_from_github():
        ...
        if USE_LOCAL_STORAGE:
            ...
            return st.session_state.demo_data, None
    ...
    def save_data_to_github(data, sha=None, file_name=DATA_FILE, retry_count=0):
        """Save data to GitHub or local storage"""
        if USE_LOCAL_STORAGE:
            # Demo mode - just update session state
        ...
        try:
            ...
            payload = {
                "message": f"Update by {st.session_state.get('name', 'Unknown')} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                "content": content,
    ...
    def load_all_data():
        """Load all data from GitHub or local storage"""
        ...
        for attempt in range(max_retries):
            ...
            for tester in testers:
                data["assignment_history"].append({
                    ...
                    "assigned_by": st.session_state.get('name', 'Unknown'),
    ...
    if 'current_user' not in st.session_state:
        st.session_state.current_user = st.session_state.get('name', 'Unknown')
...
By: {st.session_state.get('name', 'Unknown')}
...
"""
        ...
        <div class="container">
            <div class="header">
                ...
                <p>By: {st.session_state.get('name', 'Unknown')}</p>
            </div>
            <div class="content">
        ...
        html += f"""
            ...
            <div class="footer">
                <p>Task Assignment Tool v8.0 | Comprehensive Analytics Report | Generated: {now.strftime('%Y-%m-%d %H:%M:%S')}</p>
            </div>
        </div>
        </body>
        </html>
    ...
    # Multi-user warning banner (only for non-demo mode)
    if not st.session_state.get('demo_mode', False):
        ...
        if last_time != "Unknown":
            try:
                ...
                st.info(f"📝 Last modified {time_str}")
            except:
                st.info(f"📝 Last modified recently")
    ...
    with st.sidebar:
        ...
        if st.session_state.get('demo_mode', False):
            st.info("💡 In demo mode, upload any CSV to try the features")
        else:
            st.info("💡 Export from Devices and Skills Quip (Spreadsheet → Export → CSV)")
                        ...
                        with st.expander(f"📌 {task_info['name']} - {task_info['priority']} ({assignee_count} assignees)"):
                            ...
                            with col2:
                                if st.button("✅ Complete", key=f"comp_{task_id}", type="primary"):
                                    mark_task_completed(task_id, st.session_state.get('name', 'Unknown'))
                                    st.rerun()
    ...
    with col1:
        if st.session_state.get('demo_mode', False):
            st.caption("Team Task Assignment Tool v8.0 | Demo Mode | Changes are temporary")
        else:
            st.caption("Team Task Assignment Tool v8.0 | GitHub Storage | Multi-User Support")
    with col2:
